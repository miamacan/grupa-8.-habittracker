...
rectangle "Habit Tracker" {
  ...
  usecase "Oznaci kao izvrseno" as UC3
...
Proces započinje akcijom korisnika u korisničkom sučelju (UI), nakon čega se zahtjev šalje API-ju.
API provjerava u bazi podataka postoji li već unos za tu naviku i taj datum.
Ako unos već postoji, korisniku se vraća greška. U suprotnom, novi unos se sprema u bazu
i korisnik dobiva potvrdu o uspjehu.
```plantuml
@startuml
actor User
participant UI
